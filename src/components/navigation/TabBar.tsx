...
export function DbfTabBar({
  ...
  const lines = tabBarLabelLines(fontScale, barWidth, state.routes.length, longest);
  const contentHeight = tabBarContentHeight(fontScale, lines);
            ...
            {({ pressed }) => (
              ...
              >
                ...
                <Text
                  ...
                  align="center"
                  numberOfLines={lines}
                  // Two 74pt lines cannot hold "Community" at 200% — text
                  // breaks at character boundaries, so the device would render
                  // "Com" / "muni…". The label therefore stops scaling at
                  // TAB_BAR_LABEL_MAX_FONT_SCALE (the rest of the app still
                  // goes to 200%), and the full name stays in the tab's
                  // accessibilityLabel for anyone who needs it read out.
 ...
 *
 * `DbfTabBar` itself follows the active theme — that is what the gallery and
 * the component tests exercise. The shipped bar, though, sits under screens
 * that still paint a hard-coded white page, so it is pinned to the chrome
 * palette (`src/theme/chrome.ts`) for exactly as long as that is true; a dark
 * bar under a white page was the visible half of the same bug that made the
 * Android status bar disappear in dark mode.
 */
export function AppTabBar(props: BottomTabBarProps) {
  const { scheme } = useOptionalTheme();
  return <DbfTabBar {...props} scheme={chromeScheme(scheme)} />;
}
